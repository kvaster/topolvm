	...
	return corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				topols.CapacityKeyPrefix + "dc1": fmt.Sprintf("%d", cap1Gb<<30),
				topols.CapacityKeyPrefix + "dc2": fmt.Sprintf("%d", cap2Gb<<30),
				topols.CapacityKeyPrefix + "dc3": fmt.Sprintf("%d", cap3Gb<<30),
			},
		},
	}
}
	...
	testCases := []struct {
	...
	}{
		{
			input: &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						topols.CapacityKeyPrefix + "dc1": strconv.Itoa(5 << 30),
					},
				},
				Spec: corev1.PodSpec{
		...
		},
		{
			input: &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						topols.CapacityKeyPrefix + "dc1": strconv.Itoa(3 << 30),
					},
				},
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{
