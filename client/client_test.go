...
var _ = Describe("client", func() {
	...
	Context("legacy env", func() {
		...
		Context("wrappedReader", func() {
			Context("Get", func() {
				get := func(doCheck bool, f func(*wrappedReader, *topolsv1.LogicalVolume, string)) {
					i := 0
					lv := currentLV(i)
					err := k8sDelegatedClient.Create(testCtx, lv)
					Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					get(true, func(c *wrappedReader, lv *topolsv1.LogicalVolume, name string) {
				...
				It("metav1.PartialObjectMetadata", func() {
					get(false, func(c *wrappedReader, lv *topolsv1.LogicalVolume, name string) {
						p := &metav1.PartialObjectMetadata{}
						p.SetGroupVersionKind(topolsv1.GroupVersion.WithKind(kind))
			...
			Context("List", func() {
				list := func(doCheck bool, f func(*wrappedReader, *topolsv1.LogicalVolumeList)) {
					for i := 0; i < 2; i++ {
						lv := currentLV(i)
						err := k8sDelegatedClient.Create(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					list(true, func(c *wrappedReader, lvlist *topolsv1.LogicalVolumeList) {
						err := c.List(testCtx, lvlist)
						Expect(err).ShouldNot(HaveOccurred())
					})
				})
				...
				It("metav1.PartialObjectMetadata", func() {
					list(false, func(c *wrappedReader, lvlist *topolsv1.LogicalVolumeList) {
				...
				It("option test", func() {
					...
					c := NewWrappedReader(k8sAPIReader, scheme)
					err := c.List(testCtx, new(topolsv1.LogicalVolumeList), opt1, opt2)
					Expect(err).ShouldNot(HaveOccurred())
		...
		Context("wrappedClient", func() {
			Context("Get", func() {
				get := func(doCheck bool, f func(Gomega, client.Client, *topolsv1.LogicalVolume, string)) {
					i := 0
					lv := currentLV(i)
					err := k8sDelegatedClient.Create(testCtx, lv)
					Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					get(true, func(g Gomega, c client.Client, lv *topolsv1.LogicalVolume, name string) {
						err := c.Get(testCtx, types.NamespacedName{Name: name}, lv)
						g.Expect(err).ShouldNot(HaveOccurred())
					})
				})
				...
				It("metav1.PartialObjectMetadata", func() {
					get(false, func(g Gomega, c client.Client, lv *topolsv1.LogicalVolume, name string) {
						p := &metav1.PartialObjectMetadata{}
						p.SetGroupVersionKind(topolsv1.GroupVersion.WithKind(kind))
						lv.ObjectMeta.DeepCopyInto(&p.ObjectMeta)
						err := c.Get(testCtx, types.NamespacedName{Name: name}, lv)
			...
			Context("List", func() {
				list := func(doCheck bool, f func(Gomega, *wrappedReader, *topolsv1.LogicalVolumeList)) {
					for i := 0; i < 2; i++ {
						lv := currentLV(i)
						err := k8sDelegatedClient.Create(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					list(true, func(g Gomega, c *wrappedReader, lvlist *topolsv1.LogicalVolumeList) {
						err := c.List(testCtx, lvlist)
						g.Expect(err).ShouldNot(HaveOccurred())
					})
				})
				...
				It("metav1.PartialObjectMetadata", func() {
					list(false, func(g Gomega, c *wrappedReader, lvlist *topolsv1.LogicalVolumeList) {
						p := &metav1.PartialObjectMetadataList{}
						p.SetGroupVersionKind(topolsv1.GroupVersion.WithKind(kind))
						err := c.List(testCtx, lvlist)
						g.Expect(err).ShouldNot(HaveOccurred())
			...
			Context("Create", func() {
				create := func(doCheck bool, f func(client.Client, *topolsv1.LogicalVolume)) {
					i := 0
					lv := currentLV(i)
					c := NewWrappedClient(k8sDelegatedClient)
					f(c, lv)
				...
				It("typedClient", func() {
					create(true, func(c client.Client, lv *topolsv1.LogicalVolume) {
						err := c.Create(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
					})
				})
				...
				It("metav1.PartialObjectMetadata", func() {
					create(false, func(c client.Client, lv *topolsv1.LogicalVolume) {
						p := &metav1.PartialObjectMetadata{}
						p.SetGroupVersionKind(topolsv1.GroupVersion.WithKind(kind))
						lv.ObjectMeta.DeepCopyInto(&p.ObjectMeta)
			...
			Context("Delete", func() {
				delete := func(f func(client.Client, *topolsv1.LogicalVolume)) {
					i := 0
					lv := currentLV(i)
					err := k8sDelegatedClient.Create(testCtx, lv)
					Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					delete(func(c client.Client, lv *topolsv1.LogicalVolume) {
						err := c.Delete(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
					})
				})
			...
			Context("Update", func() {
				update := func(doCheck bool, f func(client.Client, *topolsv1.LogicalVolume)) {
					i := 0
					lv := currentLV(i)
					err := k8sDelegatedClient.Create(testCtx, lv)
					Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					update(true, func(c client.Client, lv *topolsv1.LogicalVolume) {
						err := c.Update(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
					})
				})
				...
				It("metav1.PartialObjectMetadata", func() {
					update(false, func(c client.Client, lv *topolsv1.LogicalVolume) {
						p := &metav1.PartialObjectMetadata{}
						p.SetGroupVersionKind(topolsv1.GroupVersion.WithKind(kind))
						lv.ObjectMeta.DeepCopyInto(&p.ObjectMeta)
			...
			Context("Patch", func() {
				patch := func(doCheck bool, f func(client.Client, *topolsv1.LogicalVolume, client.Patch)) {
					i := 0
					lv := currentLV(i)
					err := k8sDelegatedClient.Create(testCtx, lv)
					Expect(err).ShouldNot(HaveOccurred())
					...
					ann := map[string]string{"foo": "bar"}
					lv2 := lv
					lv2.TypeMeta.APIVersion = topolsv1.GroupVersion.String()
					lv3 := lv2.DeepCopy()
					lv3.Annotations = ann
				...
				It("typedClient", func() {
					patch(true, func(c client.Client, lv *topolsv1.LogicalVolume, patch client.Patch) {
				...
				It("metav1.PartialObjectMetadata", func() {
					patch(false, func(c client.Client, lv *topolsv1.LogicalVolume, patch client.Patch) {
						p := &metav1.PartialObjectMetadata{}
						p.SetGroupVersionKind(topolsv1.GroupVersion.WithKind(kind))
						lv.ObjectMeta.DeepCopyInto(&p.ObjectMeta)
			...
			Context("DeleteAllOf", func() {
				deleteAllOf := func(f func(client.Client, *topolsv1.LogicalVolume)) {
					for i := 0; i < 2; i++ {
						lv := currentLV(i)
						err := k8sDelegatedClient.Create(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
				...
				It("typedClient", func() {
					deleteAllOf(func(c client.Client, lv *topolsv1.LogicalVolume) {
						err := c.DeleteAllOf(testCtx, lv)
						Expect(err).ShouldNot(HaveOccurred())
					})
				})
